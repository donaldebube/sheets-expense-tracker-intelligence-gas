...
function setupSheets() {
  ...
  _ensureSheet(ss, SHEET_NAMES.AUDIT,      setupAuditSheet);
  refreshDashboard();
...
function _ensureSheet(ss, name, setupFn) {
  ...
  if (!sheet) { sheet = ss.insertSheet(name); setupFn(sheet); }
  return sheet;
}
...
function setupExpensesSheet(sheet) {
  ...
  // FIX: Loop through the array to set individual column widths
  const widths = [100,100,60,130,200,120,130,180];
...
function setupSettingsSheet(sheet) {
  ...
  const settings = [
    ...
    ['Currency','₦'],['Reminder Day','25'],['Name','User']
  ];
...
function setupAuditSheet(sheet) {
  sheet.clearContents();
  const headers = ['Timestamp','User','Action','Tab','Description','Old Value','New Value'];
  sheet.getRange(1,1,1,headers.length).setValues([headers])
    .setBackground('#1a1a2e').setFontColor('#ffffff').setFontWeight('bold').setFontSize(11);
  
  // FIX: Loop through the array to set individual column widths
  const widths = [160,160,80,100,260,200,200];
  widths.forEach((width, index) => {
    sheet.setColumnWidth(index + 1, width);
  });
...
function _logAudit(action, tab, description, oldValue, newValue) {
  try {
    const sheet = _getSheet(SHEET_NAMES.AUDIT);
    const user  = Session.getActiveUser().getEmail() || 'You';
    sheet.appendRow([
      new Date(), user, action, tab,
      description || '',
      oldValue    || '',
      newValue    || ''
    ]);
...
function getAuditLog() {
  ...
  return data.slice(1).reverse().slice(0, 200).map(r => ({
    timestamp:   r[0] ? new Date(r[0]).toLocaleString('en-NG') : '',
    user:        String(r[1] || ''),
    action:      String(r[2] || ''),
    tab:         String(r[3] || ''),
...
function addCategory(name, icon, color, type) {
  ...
  _logAudit('ADD', 'Categories', `Added category: ${name}`, '', `${icon} ${name} (${type})`);
  return { success: true, message: `Category "${name}" added.` };
}

function updateCategory(row, oldName, newName, icon, color, type) {
  const sheet = _getSheet(SHEET_NAMES.CATEGORIES);
  const data  = sheet.getDataRange().getValues();
  const i     = Number(row) - 1;
  if (i < 1 || i >= data.length) return { success: false, message: 'Category not found.' };
  const old = `${data[i][1]} ${data[i][0]} (${data[i][3]})`;
  sheet.getRange(row, 1, 1, 5).setValues([[newName, icon, color, type, data[i][4]]]);
  _logAudit('UPDATE', 'Categories', `Updated category: ${oldName} → ${newName}`, old, `${icon} ${newName} (${type})`);
...
function deleteCategory(name) {
  ...
  for (let i = data.length - 1; i >= 1; i--) {
    if (String(data[i][0]).toLowerCase() === name.toLowerCase()) {
      sheet.deleteRow(i + 1);
      _logAudit('DELETE', 'Categories', `Deleted category: ${name}`, `${data[i][1]} ${name}`, '');
...
function getBudgetsForMonth(month, year) {
  ...
  return data.slice(1).filter(r =>
    String(r[0]).trim() === String(month).trim() && Number(r[1]) === Number(year)
  ).map((r, i) => ({
    row:      i + 2,
    month:    r[0], year: r[1], category: r[2],
    amount:   r[3], notes: r[5] || ''
  }));
}

// Re-index rows properly (filter shifts indices)
function getBudgetsForMonthWithRows(month, year) {
  const sheet = _getSheet(SHEET_NAMES.BUDGET);
  const data  = sheet.getDataRange().getValues();
  const result = [];
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]).trim() === String(month).trim() && Number(data[i][1]) === Number(year)) {
      result.push({ row: i+1, month: data[i][0], year: data[i][1], category: data[i][2], amount: data[i][3], notes: data[i][5] || '' });
    }
  }
  return result;
}
...
function setBudget(month, year, category, amount, notes) {
  ...
  const data  = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]).trim() === String(month).trim() &&
        Number(data[i][1]) === Number(year) && data[i][2] === category) {
      const old = `₦${data[i][3]}`;
      sheet.getRange(i+1, 4, 1, 3).setValues([[Number(amount), new Date(), notes || '']]);
      _logAudit('UPDATE', 'Budget', `Updated budget: ${category} (${month} ${year})`, old, `₦${amount}`);
      return { success: true, message: `Budget updated for ${category}.` };
    }
  }
  sheet.appendRow([month, year, category, Number(amount), new Date(), notes || '']);
  _logAudit('ADD', 'Budget', `Set budget: ${category} (${month} ${year})`, '', `₦${amount}`);
  return { success: true, message: `Budget set for ${category}.` };
}

function updateBudget(row, month, year, category, amount, notes) {
  const sheet = _getSheet(SHEET_NAMES.BUDGET);
  const data  = sheet.getDataRange().getValues();
  const i     = Number(row) - 1;
  if (i < 1 || i >= data.length) return { success: false, message: 'Budget entry not found.' };
  const old = `${data[i][2]}: ₦${data[i][3]}`;
  sheet.getRange(row, 1, 1, 6).setValues([[month, year, category, Number(amount), new Date(), notes || '']]);
  _logAudit('UPDATE', 'Budget', `Updated budget: ${category} (${month} ${year})`, old, `₦${amount}`);
...
function deleteBudget(month, year, category) {
  ...
  for (let i = data.length - 1; i >= 1; i--) {
    if (String(data[i][0]).trim() === String(month).trim() &&
        Number(data[i][1]) === Number(year) && data[i][2] === category) {
      const old = `${category}: ₦${data[i][3]} (${month} ${year})`;
      sheet.deleteRow(i + 1);
      _logAudit('DELETE', 'Budget', `Deleted budget: ${category} (${month} ${year})`, old, '');
...
function addExpense(date, category, description, amount, paymentMethod, notes) {
  ...
  sheet.appendRow([d, _monthName(d), d.getFullYear(), category, description,
                   Number(amount), paymentMethod || '', notes || '']);
  _logAudit('ADD', 'Expenses',
    `Added expense: ${category} — ${description}`, '',
    `₦${amount} on ${d.toLocaleDateString('en-NG')}`);
...
function getExpensesForMonth(month, year) {
  ...
  const result = [];
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][1]).trim() === String(month).trim() && Number(data[i][2]) === Number(year)) {
      result.push({
        row:           i + 1,
...
function updateExpense(row, date, category, description, amount, paymentMethod, notes) {
  const sheet = _getSheet(SHEET_NAMES.EXPENSES);
  const data  = sheet.getDataRange().getValues();
  const i     = Number(row) - 1;
  if (i < 1 || i >= data.length) return { success: false, message: 'Expense not found.' };
  const old = `${data[i][3]}: ₦${data[i][5]} — ${data[i][4]}`;
  const d   = new Date(date);
  sheet.getRange(row, 1, 1, 8).setValues([[
    d, _monthName(d), d.getFullYear(), category,
    description, Number(amount), paymentMethod || '', notes || ''
  ]]);
  _logAudit('UPDATE', 'Expenses',
    `Updated expense: ${category} — ${description}`,
    old, `₦${amount} on ${d.toLocaleDateString('en-NG')}`);
...
function deleteExpense(row) {
  ...
  const data  = sheet.getDataRange().getValues();
  const i     = Number(row) - 1;
  const old   = i >= 1 ? `${data[i][3]}: ₦${data[i][5]} — ${data[i][4]}` : '';
  sheet.deleteRow(Number(row));
  _logAudit('DELETE', 'Expenses', `Deleted expense: ${old}`, old, '');
...
function getAssetsForMonth(month, year) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][1]).trim() === String(month).trim() && Number(data[i][2]) === Number(year)) {
      result.push({
        row: i + 1, assetName: data[i][3], assetClass: data[i][4],
        value: data[i][5], currency: data[i][6], notes: data[i][7]
      });
    }
  }
  return result;
...
function getAllAssetNames() {
  const sheet = _getSheet(SHEET_NAMES.ASSETS);
  const data  = sheet.getDataRange().getValues();
  return [...new Set(data.slice(1).map(r => r[3]).filter(Boolean))];
}
...
function upsertAsset(month, year, assetName, assetClass, value, currency, notes) {
  ...
  const data  = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][1]).trim() === String(month).trim() &&
        Number(data[i][2]) === Number(year) && data[i][3] === assetName) {
      const old = `₦${data[i][5]}`;
      sheet.getRange(i+1, 1, 1, 8).setValues([[
        new Date(), month, year, assetName, assetClass, Number(value), currency || 'NGN', notes || ''
      ]]);
      _logAudit('UPDATE', 'Assets', `Updated asset: ${assetName} (${month} ${year})`, old, `₦${value}`);
  ...
  _logAudit('ADD', 'Assets', `Added asset: ${assetName} (${month} ${year})`, '', `₦${value} — ${assetClass}`);
  return { success: true, message: `Asset "${assetName}" added.` };
}

function updateAsset(row, month, year, assetName, assetClass, value, currency, notes) {
  const sheet = _getSheet(SHEET_NAMES.ASSETS);
  const data  = sheet.getDataRange().getValues();
  const i     = Number(row) - 1;
  if (i < 1 || i >= data.length) return { success: false, message: 'Asset not found.' };
  const old = `${data[i][3]}: ₦${data[i][5]}`;
  sheet.getRange(row, 1, 1, 8).setValues([[
    new Date(), month, year, assetName, assetClass, Number(value), currency || 'NGN', notes || ''
  ]]);
  _logAudit('UPDATE', 'Assets', `Updated asset: ${assetName} (${month} ${year})`, old, `₦${value}`);
...
function deleteAsset(row) {
  ...
  const data  = sheet.getDataRange().getValues();
  const i     = Number(row) - 1;
  const old   = i >= 1 ? `${data[i][3]}: ₦${data[i][5]}` : '';
  sheet.deleteRow(Number(row));
  _logAudit('DELETE', 'Assets', `Deleted asset: ${old}`, old, '');
...
function saveSettings(email, currency, reminderDay, name) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (map[data[i][0]] !== undefined) sheet.getRange(i+1, 2).setValue(map[data[i][0]]);
  }
  _logAudit('UPDATE', 'Settings', 'Updated settings', '', `Name: ${name}, Email: ${email}`);
...
function getDashboardData(month, year) {
  try {
    const expenses = getExpensesForMonth(month, year); // now returns safe dates
    ...
    const qMonths      = [0,1,2].map(i => _monthName(new Date(Number(year), qStart + i, 1)));
    const allExpData   = _getSheet(SHEET_NAMES.EXPENSES).getDataRange().getValues().slice(1);
    const quarterlySpend = allExpData
      .filter(r => qMonths.includes(String(r[1]).trim()) && Number(r[2]) === Number(year))
      .reduce((s, r) => s + Number(r[5]), 0);

    const breakdown = cats.map(c => ({
      name:     String(c.name  || ''),
      icon:     String(c.icon  || '📦'),
      color:    String(c.color || '#9E9E9E'),
      type:     String(c.type  || 'Variable'),
      spent:    Number(byCat[c.name]     || 0),
      budgeted: Number(budgetMap[c.name] || 0)
    })).filter(c => c.spent > 0 || c.budgeted > 0);

    const trend       = _getLast6MonthsTrend(month, year);
    const savingsRate = totalBudgeted > 0
      ? Number(Math.max(0, ((totalBudgeted - totalSpent) / totalBudgeted) * 100).toFixed(1))
      : 0;
    const topSpends = Object.entries(byCat)
      .sort((a, b) => b[1] - a[1]).slice(0, 5)
      .map(([cat, amt]) => ({ category: String(cat), amount: Number(amt) }));

    // Sanitise assets — no raw Date objects
    ...
    // Note: we do NOT return the raw `expenses` array here — it's not needed by the overview
    return {
      totalSpent, totalBudgeted, totalAssets, quarterlySpend,
      breakdown, trend, savingsRate, topSpends,
      assets: assetSummary
    };
    ...
    return {
      totalSpent: 0, totalBudgeted: 0, totalAssets: 0, quarterlySpend: 0,
      breakdown: [], trend: [], savingsRate: 0, topSpends: [], assets: [],
...
function _getLast6MonthsTrend(currentMonth, year) {
  ...
  const months = [];
  const d      = new Date(`${currentMonth} 1, ${year}`);
  for (let i = 5; i >= 0; i--) {
    const m = new Date(d.getFullYear(), d.getMonth() - i, 1);
    months.push({ month: _monthName(m), year: m.getFullYear(),
                  label: `${_monthName(m).slice(0,3)} ${m.getFullYear()}` });
  }
  return months.map(m => {
    const total = data
      .filter(r => String(r[1]).trim() === m.month && Number(r[2]) === m.year)
      .reduce((s,r) => s + Number(r[5]), 0);
...
function sendMonthlyReminderEmail() {
  ...
  const now       = new Date();
  const nextMonth = _monthName(new Date(now.getFullYear(), now.getMonth() + 1, 1));
  ...
  const body    = `
    <div style="font-family:'Helvetica Neue',sans-serif;max-width:600px;margin:0 auto;background:#f8f9fa;padding:20px;">
      ...
      <div style="background:white;padding:30px;border-radius:0 0 12px 12px;box-shadow:0 4px 6px rgba(0,0,0,0.1);">
        ...
        <p style="color:#555;line-height:1.6;">Time to set your budget for <strong>${nextMonth} ${year}</strong>.</p>
        ...
        <p style="color:#888;font-size:13px;text-align:center;">Sent automatically from your Expense Tracker.</p>
      </div>
    </div>`;
...
function scheduleMonthlyReminder() {
  ...
  ScriptApp.newTrigger('sendMonthlyReminderEmail').timeBased().onMonthDay(day).atHour(9).create();
  return { success: true, message: `Monthly reminder scheduled for day ${day} of each month.` };
}

// ─────────────────────────────────────────────────────────────
// DASHBOARD SHEET REFRESH
// ─────────────────────────────────────────────────────────────

function refreshDashboard(month, year) {
  if (!month || !year) {
    const now = new Date();
    month = _monthName(now);
  ...
  const dark = '#1a1a2e', white = '#ffffff', light = '#f8f9ff';
  ...
  kpis.forEach((kpi, i) => {
    ...
    sheet.getRange(kpiRow,   col, 1, 2).merge().setValue(kpi[0])
      .setBackground(dark).setFontColor(white).setFontWeight('bold').setFontSize(10).setHorizontalAlignment('center');
  ...
  const budRow = 6;
  sheet.getRange(budRow,   1,1,6).merge().setValue('💰 BUDGET vs ACTUAL')
    .setBackground(dark).setFontColor(white).setFontWeight('bold').setFontSize(11).setHorizontalAlignment('center');
  ...
  data.breakdown.forEach(c => {
    ...
    sheet.getRange(r, 1,1,6).setValues([[c.icon+' '+c.name, c.type, c.budgeted, c.spent, rem, status]]).setBackground(bg);
    sheet.getRange(r, 3,1,3).setNumberFormat('#,##0');
    r++;
  });

  const trendRow = r + 2;
  sheet.getRange(trendRow,   1,1,6).merge().setValue('📈 6-MONTH SPENDING TREND')
    .setBackground(dark).setFontColor(white).setFontWeight('bold').setFontSize(11).setHorizontalAlignment('center');
  sheet.getRange(trendRow+1, 1,1,6).setValues([data.trend.map(t => t.label)])
    .setBackground('#2d2d44').setFontColor(white).setFontWeight('bold').setHorizontalAlignment('center');
  sheet.getRange(trendRow+2, 1,1,6).setValues([data.trend.map(t => t.total)])
    .setBackground(light).setNumberFormat('#,##0').setHorizontalAlignment('center');

  const assRow = trendRow + 5;
  sheet.getRange(assRow,   1,1,4).merge().setValue(`💼 ASSET PORTFOLIO — ${month} ${year}`)
    .setBackground(dark).setFontColor(white).setFontWeight('bold').setFontSize(11).setHorizontalAlignment('center');
  ...
  const topRow = assRow;
  sheet.getRange(topRow,   6,1,4).merge().setValue('🔥 TOP SPENDING AREAS')
    .setBackground(dark).setFontColor(white).setFontWeight('bold').setFontSize(11).setHorizontalAlignment('center');
  ...
  data.topSpends.forEach((s, i) => {
    ...
    const tip = s.amount > (data.totalBudgeted * 0.25) ? ' ⚠️' : '';
    sheet.getRange(tr, 6,1,4).setValues([[i+1, s.category+tip, s.amount, pct]])
      .setBackground(i % 2 === 0 ? light : white);
  ...
  sheet.getRange(ar+2, 1,1,6).merge()
    .setValue(`Last refreshed: ${new Date().toLocaleString()} — ${month} ${year}`)
